...
const CreateSepidarQuery = require('./NewModule/CreateSepidarQuery');
...
exports.pay = async (req, res) => {
    if(req.query.userid) {
        ...
        try{
            payRequestResult = await bpPayRequest(newOrder.faktorNo,
                parseInt(userid=="6725d8e52602bb5d4dac0262"?"10000":
                    newOrder.totalPrice), 'ok', callbackUrl);
        }
        catch{
    ...
    if(resultCode_bpPayRequest === 0) {
        ...
        if(parseInt(resultCode_bpVerifyRequest) === 0 || resultCode_bpinquiryRequest === 0) {
            ...
            if(resultCode_bpSettleRequest === 0 || resultCode_bpSettleRequest === 45) {
                ...
                await PayLogSchema.create(query)
                const cartDetails = await faktor.findOne({faktorNo:saleOrderId})
                const userDetail = cartDetails&&await customers.findOne({_id:ObjectID(cartDetails.userId)})
                await transaction.create(query)
                await CreateTask('web', cartDetails, userDetail);

                const cartItems = await faktorItemModel.find({faktorNo:saleOrderId})
                const sepidarQuery = CreateSepidarQuery(cartDetails,cartItems, saleOrderId)
                
                const sepidarResult = await sepidarPOST(sepidarQuery, "/api/invoices", cartDetails.userId)
                await faktor.updateOne({faktorNo:saleOrderId},
                    {$set:{payStatus:"paid",sepidarResult:sepidarResult,
                        query:sepidarQuery,
                        InvoiceNumber:sepidarResult.Number,
