...
const panelQuoteApi = require('./quoteApiOld')
...
router.use('/panel/brands', require('./brands'));
router.use('/panel/stocks', require('./stocks'));
router.use('/panel/menus', require('./menus'));
router.use('/panel/crm', CRMPanelApi)


router.use('/web/user', require('./Web/userApi'));
router.use('/web/product', require('./Web/productApi'));
...
router.post('/sepidar-product', async (req, res) => {
    ...
    try {
        ...
        var tempSku=''
        await products.updateMany({},{$set:{active:false}})
        for (var i = 0; i < sepidarResult.length; i++) {
            tempSku = sepidarResult[i].Code
        ...
        res.status(500).json({ message: error.message ,sku:tempSku})
    }
})
        ...
        for (var i = 0; i < sepidarResult.length; i++) {
            ...
            const custResult = await customers.updateOne({
                ...
                $set: {
                    ...
                    AddressID: mainAddress?mainAddress.CustomerAddressID : '',
...
router.post('/sepidar-update-log',auth, async (req, res) => {
    try {
        const userData = await users.findOne({_id:ObjectID(req.headers['userid'])})
        if(!userData){
            res.status(400).json({error:"error not found"})
        ...
        var fAccess = userData.access=="manager"?1:0
        var StockTemp = (fAccess&&req.body.stock)?req.body.stock:''
        var Stock = StockTemp?StockTemp:userData.StockId
        const countLog = await updateLog.aggregate([
            {$match:{updateQuery:"sepidar-quantity"}},
            {$match:!fAccess?{$or:[
                {Stock:{$exists:false}},
