...
router.post('/login', jsonParser, async (req, res) => {
  try {
    ...
    if (user.profileId) {
      const userProfile = await profileModel.findOne({ _id: user.profileId }).lean();
      if (userProfile) {
        const listOfmenuCodes = userProfile.menusAccess.map(menu => menu.menuCode);
        const menus = await menuModel
          .find({ menuCode: { $in: listOfmenuCodes } })
          .select({ _id: 0, __v: 0, updatedAt: 0, createdAt: 0})
          .lean();
        const parentsList = menus.map((m) => m.parent);
        const parentsListInfo = await menuModel.find({ menuCode: { $in: parentsList } }).lean();
        let parents = parentsListInfo.map((p) => {
          return {
            menuCode: p.menuCode,
            title: p.title,
            enTitle: p.enTitle,
        ...
        menus.forEach((m) => {
          if (m.parent) {
            parents = parents.map((p) => {
              if (p.menuCode === m.parent) {
                const access = userProfile?.menusAccess?.find((a) => a.menuCode === m.menuCode)?.access || null;
                if (access.read) {
                  p.list.push({
                    ...m,
                    access,
        ...
        user.menus = parents.filter(p => p.list && p.list.length > 0);
      }
    }
...
router.post('/sendOtp', jsonParser, async (req, res) => {
  ...
  try {
    ...
    if (user) {
      ...
      res.status(200).json({
        message: "welcome to sharif, sms sent for " + phone,
        smsResult: smsResult
      });
    }
  }
    ...
    res.status(400).json({
      message: "login error", error: error,
      smsResult: smsResult
    });
  }
})
  ...
  try {
    ...
    if (!user) {
      ...
      const user = //bitrixData.result&&
        await User.create({
          ...data, bitrixCode: bitrixData.result,
          otp: newOtp
        });
...
router.post('/list-search', auth, jsonParser, async (req, res) => {
  try {
    ...
    res.status(200).json({ user: req.user, message: "User List"})
