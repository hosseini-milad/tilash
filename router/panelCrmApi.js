...
const calcTasks = async (userId) => {
		...
		if (userData.access == 'admin') {
            access = 7; 
            const userList = await user.find({ profile: { $in: userData.profile } });
		...
		const crmData = await crmlist.findOne().lean();
        ...
        if (access < 8) {
            tasksMatchCondition.creator = { $in: myCreator };
        }
		let taskList = await tasks.aggregate([
			//{$match:limitTask?{profile:limitTask}:{}},
			{ $match: tasksMatchCondition },
            {
                $addFields: {
                    tempTimestamp: { $ifNull: ['$progressDate', '$date'] },
                },
            },
			{
				$addFields: {
					taskStep: {
						$cond: {
							if: {
								$and: [{ $in: ['$taskStep', ['archive', 'cancel']] }, { $lt: ['$tempTimestamp', new Date(new Date() - 2 * 24 * 60 * 60 * 1000)] }],
							},
							then: null,
							else: '$taskStep',
						},
					},
					user_Id: { $convert: { input: '$assign', to: 'objectId', onError: '', onNull: '' } },
					profile_Id: { $convert: { input: '$profile', to: 'objectId', onError: '', onNull: '' } },
					creator_Id: { $convert: { input: '$creator', to: 'objectId', onError: '', onNull: '' } },
					customer_Id: { $convert: { input: '$customer', to: 'objectId', onError: '', onNull: '' } },
				},
			},
            { $match: { taskStep: { $ne: null } } },
			{ $lookup: { from: 'users', localField: 'user_Id', foreignField: '_id', as: 'userInfo' } },
			{ $lookup: { from: 'profiles', localField: 'profile_Id', foreignField: '_id', as: 'profileInfo' } },
		...
		return {
            ...
            tasks: tasksToShow,
            crm: crmData,
            columnOrder: showColumn,
            columns,
            userData:userData?userData:"not set",
            myCreator
        };
        try {} catch (error) {
		return({error:error});
    ...
    if (status === "sepidar") {
        ...
        if (sepidarResult&&sepidarResult.Message){
            ...
            await tasks.updateOne({ _id: ObjectID(taskId) },
                {
                    $set: {
                        taskStep: "done", query: sepidarQuery,
                        result: sepidarResult, progressDate: Date.now()
                    }
                })
            await cart.updateOne({ cartNo: taskData.orderNo},
                {$set:{status:"undone"} })
            res.status(400).json({error:errorSep , message:errorSep})
        }
        else{
        ...
        await tasks.updateOne({ _id: ObjectID(taskId) },
            {
                $set: {
                    taskStep: "archive", query: sepidarQuery,
                    result: sepidarResult, progressDate: Date.now()
                }
            })
        }
...
router.post('/update-bulk', auth, jsonParser, async (req, res) => {
    ...
    for (var i = 0; i < orderList.length; i++) {
        ...
        if (taskData && taskData.taskStep == "done") {
            const sepidarResult = await SepidarOrder(orderList[i],1)
...
const checkAccess = (req, res, next) => {
    try {
        const API_KEY = '1b5791178451a7fca86a52044cdfac07';
        ...
        return res.sendStatus(500).json({ message: 'خطای داخلی سرور' });
    }
};

router.post('/update-sending', checkAccess, jsonParser, async (req, res) => {
    try {
        const { orderNo } = req.body;
        if (!orderNo) {
            return res.status(400).json({ error: "شناسه سفارش الزامی است." });
        }
        const targetCart = await tasks.findOne({ orderNo }).lean();
        if (!targetCart) {
            return res.status(400).json({ error: "سفارش یافت نشد." });
        }
        const updateCart = await tasks.updateOne({ orderNo }, { $set: { taskStep: "done" } });
        return res.json({ message: "وضعیت سفارش به 'انجام شده' تغییر یافت." });
    } catch (err) {
        return res.sendStatus(500);
    }
});

router.get('/find-sending', checkAccess, jsonParser, async (req, res) => {
    try {
        // let orderList = req.body.orders;
        const status = 'sending'; // req.body.status;
        // if (!orderList || !orderList.length) {
        // }
        let orderListTemp = await tasks.find({ taskStep: status }).lean();
        console.log('orderListTemp: ', orderListTemp.length);
        let orderList = orderListTemp.map(item => item.orderNo);
