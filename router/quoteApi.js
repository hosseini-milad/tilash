    ...
    try {
        ...
        if (qCartData) {
            ...
            qCartDetail = findQuickCartSum(qCartData.cartItems,
                qCartData.payValue, qCartData.discount,qCartData.transportPrice)
        }
        return ({
            cart: todayCartData, cartDetail: cartDetail, userData: "userData", isSale,policy:true,
            quickCart: qCartData, qCartDetail: qCartDetail, qCartAdmin: qCartAdmin,needDependency:false
        })
    }
    catch {
        return ({
...
const findCartSum = (cartItems, payValue) => {
    ...
    return ({
        ...
        totalPrice: (cartSum * (1 + Number(TaxRate)) - cartDiscount),
        cartDescription: cartDescription
    })
}
                    ...
                    if (oldCartItems[i]?.price.length) {
                        for (let j = 0; j < oldCartItems[i].price.length; j++) {
                            if (oldCartItems[i].price[j].saleType === CartData.payValue) {
                                ...
                                oldCartItems[i].price[j].price = `${data.changes.price}`;
                                oldCartItems[i].hasNewPrice = true;
                                oldCartItems[i].priceUpdatedAt = new Date();
                            }
                        }
                    } else {
                        oldCartItems[i].originalPrice = [];
                        if (!oldCartItems[i].price) {
                            oldCartItems[i].price = [];
                        }
                        oldCartItems[i].price[0] = {
                            price: `${data.changes.price}`,
                            ItemID: oldCartItems[i].id,
                            saleType: CartData.payValue
                        };
                        oldCartItems[i].hasNewPrice = true;
                        oldCartItems[i].priceUpdatedAt = new Date();
                    }
            ...
            notNullCartItem.map((item, i) => (
                {
                    ...
                    "Tax": normalPriceCount(item.price, item.count, Number(TaxRate)),
                    "Duty": 0.0000,
                    "Addition": 0.0000
                }))
...
router.post('/edit-payValue', jsonParser, async (req, res) => {
	try {
		...
		let status = '';
		cartNo ? await quoteApi.updateOne({ cartNo }, { $set: { payValue: req.body.payValue } }) : await quickCart.updateOne({ userId: data.userId }, { $set: data });
		status = 'update cart';
        ...
        const response = {
            ...cartDetails,policy:true,needDependency:false,
