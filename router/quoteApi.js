...
const qCart = require('../models/product/quote');
...
const quickCart = require('../models/product/quote');
...
const quickCartModel = require('../models/product/quote');
...
router.post('/delete-discount', jsonParser,auth, async (req, res) => {
    try {
        ...
        if(cartNo) userId = cartData.userId
        ...
        const cartDetails = await findCartFunction(userId, req.headers['userid']); // TODO
        return res.json({message:"تخفیف حذف شد",...cartDetails})
    }
    catch(error){
...
router.post('/quick-to-cart', jsonParser, async (req, res) => {
    try {
        ...
        const { branchName, branchId, date,  transport ,bankArray,
                transportPrice,bank,bankDate,cartID, isQuote=1 } = req.body;
        var now = new Date()
