...
router.post('/findCount',async(req,res)=>{
    const sku = req.body.sku
    const stock = req.body.stock
    const saleType = req.body.saleType
    var count = await CalcCount(sku,stock?stock:"13",saleType?saleType:"4")
    res.json(count)
})
router.post('/addToCart',auth, async (req,res)=>{
    ...
    try{
        ...
        var productInfo = await CalcCount(sku)
        const productData = productInfo.data
        //console.log(productInfo)
        if(!productInfo)return res.status(400).json({error:"خطای محصول"})
        if(!productInfo.count)
            return res.status(400).json({error:"محصول موجود نیست"})
        if(!productInfo.price)
            return res.status(400).json({error:"محصول قیمت ندارد"})
...
router.get('/cart-detail',auth, async (req,res)=>{
    ...
    try{
        ...
        var priceSet=[]
        var totalCartData = await CalcCart(cartDetails,"35")
